...
router.post("/statements/parse", upload.single("statement"), async (req, res): Promise<void> => {
  if (!req.file) {
...
- Return ONLY valid JSON array, nothing else`;
  ...
  try {
    ...
    if (!jsonMatch) {
      res.status(422).json({ error: "Could not extract transactions from statement. Make sure the image is clear and shows transaction data." });
      return;
    }
    ...
    res.status(422).json({ error: "Could not parse the extracted transactions. Please try again with a clearer image." });
    return;
  }
