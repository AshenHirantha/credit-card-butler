...
import { useListCards, useCreateCard, useDeleteCard, getListCardsQueryKey, getGetDashboardQueryKey } from "@workspace/api-client-react";
import { useQueryClient } from "@tanstack/react-query";
import { formatCurrency, getUtilizationColor } from "@/lib/format";
import { Card as CardUI, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Progress } from "@/components/ui/progress";
import { PlusCircle, Trash2, CreditCard, Calendar } from "lucide-react";
...
const cardSchema = z.object({
  ...
  color: z.string().regex(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/, "Must be a valid hex color").optional().or(z.literal("")),
});
...
export default function CardsPage() {
  ...
  function onSubmit(values: z.infer<typeof cardSchema>) {
    createCard.mutate({
      ...
      onError: (error) => {
        toast({ title: "Failed to add card", description: error.message || "An error occurred", variant: "destructive" });
      }
    });
  }
  ...
  function handleDelete(id: number) {
    deleteCard.mutate({ id }, {
      ...
      onError: (error) => {
        toast({ title: "Failed to delete card", description: error.message || "An error occurred", variant: "destructive" });
      }
    });
  }
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="e.g. Sapphire Reserve" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <div className="flex gap-2">
                        ...
                        <FormControl>
                          <Input className="flex-1" placeholder="#000000" {...field} />
                        </FormControl>
                      </div>
                      <FormMessage />
                    </FormItem>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 bg-muted/20 border rounded-xl">
          ...
          <p className="text-muted-foreground mt-2">Add your first credit card to start tracking.</p>
        </div>
      ) : (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3">
          {cards.map((card) => (
            <CardUI key={card.id} className="flex flex-col relative overflow-hidden group">
              {card.color && (
                <div 
              ...
              <CardHeader className="pb-4 border-b bg-muted/10 relative z-10">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    <div 
                      className="w-10 h-10 rounded flex items-center justify-center shrink-0 shadow-sm border bg-card"
                    >
                      <CreditCard className="w-5 h-5" style={{ color: card.color || 'var(--foreground)' }} />
                    </div>
                    <div>
                      <CardTitle className="text-lg">{card.name}</CardTitle>
                      <p className="text-sm text-muted-foreground">{card.bank || 'No Bank'}</p>
                    </div>
                  </div>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          ...
                          All associated transactions will be kept but unlinked from this card.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
              ...
              <CardContent className="pt-6 flex-1 flex flex-col gap-6 relative z-10">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Used: <span className="font-medium text-foreground">{formatCurrency(card.used)}</span></span>
                    <span className="font-medium">{Math.round(card.utilization * 100)}%</span>
                  </div>
                  <Progress 
                ...
                <div className="flex items-center gap-4 text-sm pt-4 border-t mt-auto">
                  ...
                  <div className="flex items-center gap-1.5 font-medium">
                    ...
                    <span>Due: {card.dueDay} ({card.daysUntilDue}d)</span>
                  </div>
                </div>
              </CardContent>
            </CardUI>
